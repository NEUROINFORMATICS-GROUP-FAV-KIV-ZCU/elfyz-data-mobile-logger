...
public class GlucoseMeterListFragment extends ADataListFragment {
  private static final int LOADER_ID = 42;
  

  @Override
  public Loader<AsyncTaskResult<Cursor>> onCreateLoader(int id, Bundle args) {
    return new GMeasurementLoader(getActivity(), userID);
  }
  ...
  protected ARecordTable<?> getDatabase() {
    return Application.getInstance().getDatabase().getGlucoseMeasurementTable();
  }
  ...
  protected IExperimentParametersUploadHelper getUploadHelper(long[] selected) {
    ...
    return new GlucoseMeasurementDbUploadHelper(parameterName, 0.0, selected, append);
  }

  @Override
  protected CursorAdapter getDataAdapter(Context context) {
    return new GlucoseMeterAdapter(getActivity(), null, 0);
  }

  
  public static Fragment newInstance(long userID) {
    Fragment instance = new GlucoseMeterListFragment();
